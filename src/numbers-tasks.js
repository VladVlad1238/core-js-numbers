...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const getCoords = (x2 - x1) ** 2 + (y2 - y1) ** 2;
  return Math.sqrt(getCoords);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const coordSum = x1 * x2 + y1 * y2;

  const firstMagnitude = Math.sqrt(x1 ** 2 + y1 ** 2);
  const secondMagnitude = Math.sqrt(x2 ** 2 + y2 ** 2);

  const cosinus = coordSum / (firstMagnitude * secondMagnitude);
...
function toNumber(value, def) {
  ...
  return typeof converToNumber === 'number' && !Number.isNaN(converToNumber)
    ? converToNumber
    : def;
}
...
function getCountOfOddNumbers(number) {
  let coutOfOddNumber = 0;
  const absNumber = Math.abs(number);

  for (let i = 1; i <= absNumber; i += 1) {
    if (i % 2 !== 0) {
      coutOfOddNumber += 1;
    }
  }
  return coutOfOddNumber;
}
